  ...
  "units": [
    {
      ...
      "include": ["*For_Redux*"],      // опц. (kind=folder): брать ТОЛЬКО эти архивы
      "exclude": ["ZelDomiksOld*"],    // опц. (kind=folder): эти архивы НЕ брать
...
`include`/`exclude` — glob-маски по имени архива, применяются к архивам-братьям в
раздаче-папке. `include` нужен, когда в одной папке лежат варианты пака под разные базы
(`Solyanka…For_Redux` vs `…For_Original`) — без фильтра оба слились бы в один юнит.
`exclude` — когда рядом с обычными модами лежит АЛЬТЕРНАТИВНАЯ версия одного из них: её
выносят во второй юнит поверх той же раздачи (`zelmods` + `zelmods_old`), чтобы у игрока
был **выбор версии** в лаунчере, а не молчаливое схлопывание в одну. Именно `exclude`, а не
перечисление остальных через `include`: новый архив, добавленный автором раздачи, продолжит
ехать в основной юнит, а не выпадет из каталога. `exclude` сильнее `include`.
...
Код пайплайна — **GNU GPL v3 или новее**, полный текст в [LICENSE](LICENSE).
Copyright (C) 2026 ArtYudin89.

Лицензия покрывает **только мой код**: `pipeline/`, `tools/` (кроме вендоренных
сторонних утилит), скрипты сборки и формат дескрипторов. Она **не покрывает**:

- **сами моды** в `mods/` и ассеты на Hugging Face — они принадлежат своим авторам и
  лежат здесь как зеркало ради воспроизводимости каталога; условия распространения
  спрашивайте у авторов модов;
- **ресурсы игры** — принадлежат правообладателям, в репозитории их нет и быть не должно;
- **вендоренные утилиты** (`tools/innounp/` и подобные) — под своими лицензиями.

## Поддержать

USDT и прочие способы — в [DONATE.md](DONATE.md). Проект бесплатный, донат ни на что не влияет.
